# Guide de Dépannage pour l'Interface Tkinter

Ce guide vous aidera à résoudre les problèmes liés à l'affichage de l'interface Tkinter lorsque vous cliquez sur le bouton "Voir" pour visualiser vos données.

## Problème

Lorsque vous cliquez sur le bouton "Voir" pour visualiser vos données, l'interface Tkinter ne s'affiche pas. Les logs de la console montrent que le backend tente de lancer l'interface, mais celle-ci ne s'affiche pas. Cela est généralement dû à un problème de configuration X11, en particulier dans les environnements Docker.

## Cause Racine

Le problème est lié à l'absence ou à la mauvaise configuration de la variable d'environnement `DISPLAY`, qui est nécessaire pour que les applications graphiques comme Tkinter puissent s'afficher. Dans les environnements Docker, le conteneur peut ne pas avoir accès au serveur X11 de l'hôte, ce qui empêche l'interface graphique de s'afficher.
...
2. **Si vous utilisez Docker Compose**, assurez-vous que les options suivantes sont ajoutées à votre service backend dans le fichier `docker-compose.yml`:
   ```yaml
   services:
     backend:
       environment:
...
3. **Exécutez le script de test X11** pour vérifier la configuration:
   ```bash
   python test_x11_config.py
   ```
   Ce script vérifie la configuration X11 et tente de créer une fenêtre Tkinter simple.
...
Pour plus de détails, consultez le guide complet [X11_SETUP_GUIDE.md](X11_SETUP_GUIDE.md).
